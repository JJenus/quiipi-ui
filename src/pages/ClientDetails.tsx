...
import { useClient, useClients, useClientProjects, useClientInvoices, useClientSubscriptions } from '@/hooks/useClients';
...
import { InvoiceStatus, ProjectStatus, SubscriptionType, ClientStatus } from '@/types';
...
export const ClientDetails: React.FC = () => {
  ...
  const { subscriptions, isLoading: subscriptionsLoading } = useClientSubscriptions(id!);
  ...
  const getStatusColor = (status: ClientStatus) => {
    const colors = {
      [ClientStatus.ACTIVE]: 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300',
      [ClientStatus.INACTIVE]: 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300',
      [ClientStatus.LEAD]: 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300',
    };
  ...
  const totalOutstanding = invoices?.reduce((sum, inv) => 
    sum + (inv.status !== InvoiceStatus.PAID ? inv.pendingBalance : 0), 0
  ) || 0;
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
        <Card className="bg-blue-50 dark:bg-blue-950/20">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Across {invoices?.filter(i => i.status !== InvoiceStatus.PAID).length || 0} invoices
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-red-50 dark:bg-red-950/20">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {formatCurrency(
                overdueInvoices.reduce((sum, inv) => sum + inv.pendingBalance, 0),
                client.currency
              )}
            </p>
          </CardContent>
          ...
          <TabsContent value="overview" className="space-y-4 mt-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <a href={`mailto:${client.email}`} className="text-sm break-all hover:underline">
                    {client.email}
                  </a>
                </div>
                <div className="flex items-center gap-3">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium">{client.taxId || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Payment Terms:</span>
                  <span className="text-sm font-medium">{client.paymentTerms || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Currency:</span>
                  <span className="text-sm font-medium">{client.currency}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Credit Limit:</span>
                  <span className="text-sm font-medium">
                    {formatCurrency(client.creditLimit || 0, client.currency)}
                  </span>
                </div>
                {client.notes && (
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button 
                variant="outline" 
                className="w-full"
                onClick={() => navigate('/projects/new', { state: { clientId: id } })}
              >
                ...
                {projectsLoading ? (
                  ...
                  <div className="space-y-3">
                    {projects?.map((project) => {
                      const isNearDeadline = getDaysUntil(project.deadline) <= 7 && getDaysUntil(project.deadline) > 0;
                        ...
                        >
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                {isNearDeadline && (
                                  <Badge variant="destructive" className="animate-pulse">
                                    {getDaysUntil(project.deadline)} days left
                                  </Badge>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm mt-3">
                            ...
                            <div>
                              ...
                              <p className={`font-medium ${isNearDeadline ? 'text-red-600' : ''}`}>
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm mt-3">
                          <div>
                            ...
                            <p className={invoice.status === InvoiceStatus.OVERDUE ? 'text-red-600 font-medium' : ''}>
          ...
          <TabsContent value="subscriptions" className="mt-4">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button 
                  size="sm" 
                  onClick={() => navigate('/subscriptions/new', { state: { clientId: id } })}
                >
                  <Plus className="mr-2 h-4 w-4" />
                  Add
                </Button>
                          ...
                          >
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-muted-foreground mt-1">
                                Deadline: {formatDate(project.deadline, 'MMM d, yyyy')}
                              </p>
                            </div>
                            <div className="text-right">
                              <p className="font-bold">{formatCurrency(project.estimatedBudget || 0)}</p>
                              <p className="text-xs text-muted-foreground">Budget</p>
                            </div>
                          </div>
                        );
                        ...
                        return (
                          ...
                          >
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-muted-foreground mt-1">
                                {sub.provider} • Expires: {formatDate(sub.expiryDate, 'MMM d, yyyy')}
                              </p>
                            </div>
                            <div className="text-right">
                              <p className="font-bold">{formatCurrency(sub.cost, sub.currency)}</p>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              This will also delete all associated projects, invoices, and subscriptions.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
