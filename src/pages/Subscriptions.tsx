...
export const Subscriptions: React.FC = () => {
	...
	const { subscriptions: expiringSubscriptions } =
		useExpiringSubscriptions(30);
	...
	const tabs = [
		...
		},
		{
			value: "domains",
			label: "Domains",
			count: filteredSubscriptions.filter((s) => s.type === "DOMAIN")
				.length,
			icon: Globe,
		},
		{
			value: "hosting",
			label: "Hosting",
			count: filteredSubscriptions.filter((s) => s.type === "HOSTING")
				.length,
			icon: Server,
		},
	];
			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
				<div>
					...
					<p className="text-sm sm:text-base text-muted-foreground">
						Track and manage all your subscriptions, domains, and
						hosting services.
					</p>
				</div>
				...
				>
					...
					<AlertDescription className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
						<span>
							...
							{expiringSubscriptions.length > 1 ? "s" : ""}{" "}
